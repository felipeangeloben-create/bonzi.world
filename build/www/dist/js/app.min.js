    ...
    function() {
        ...
        function() {
            ...
            function SidebarDirectoryController($rootScope, $scope, $log, $http, $timeout, $interval, $window, socket, Room) {
                ...
                }, vm.rooms = [], socket.emit("rooms:get", function(list) {
                    vm.rooms = list || [];
                    $scope.initialized = !0
                }), $scope.updateRooms = function() {
                    socket.emit("rooms:get", function(list) {
                        vm.rooms = list || []
                    })
                    ...
                    if (rm) {
                        ...
                        if (!me) {
                            socket.emit("room:join", {
                                room: rm.code || rm._id || rm
                            }, function(data) {
                                data && data.success && (window.showToast("success", "Joining room."), $scope.$emit("sidebar:close"))
                            });
                            return;
                        }
                        me.exit(function() {
                            ...
                            passcode && (pl.passcode = passcode), vm.clearBonzis(), socket.emit("room:join", pl, function(data) {
                                ...
                                $scope.enteredPasscode = "", window.showToast("success", "Joining room."), $scope.$emit("sidebar:close"), $timeout(function() {
                                    var fresh = window.bonzis[window.guid || window.identity.guid] || self;
                                    fresh.updateSprite(!1), fresh.enter()
                                }, 1e3)
                            })
                        }.bind(me))
                    }
            ...
            function ChatLogController($rootScope, $scope, $log, $compile, $http, $timeout, $interval, $window, socket) {
                ...
                $rootScope.muteAudio = vm.status.muted, $rootScope.backgroundMuted = vm.status.backgroundMuted, $scope.options = {
                    ...
                    maxMessageCount: 125
                        ...
                        args = args || {}, payload.type = payload.type || "text", payload.body = text, payload.text = text, payload._bonzi = _.get(window.bonzis, payload.guid) || null;
                        payload.bonzi = payload.bonzi || {};
                        payload.bonzi.name = payload.name || (payload._bonzi && payload._bonzi.userPublic && payload._bonzi.userPublic.name) || (payload._bonzi && payload._bonzi.public && payload._bonzi.public.name) || (payload.bonzi && payload.bonzi.name) || "";
                        payload.bonzi.color = payload.bonzi.color || (payload._bonzi && payload._bonzi.public && payload._bonzi.public.color) || null;
                        payload.bonzi.uuid = payload.bonzi.uuid || payload.guid || null;
                        payload.bonzi.guid = payload.guid || null;
                        payload.style = {
                    ...
                    socket.on("image", data => {
                        ...
                        var _bonzi = _.get(window.bonzis, data.guid) || null;
                        var pl = {
                            type: "media",
                            subtype: "image",
                            guid: data.guid,
                            _bonzi: _bonzi,
                            bonzi: {
                                name: (_bonzi && _bonzi.userPublic && _bonzi.userPublic.name) || "",
                                color: (_bonzi && _bonzi.userPublic && _bonzi.userPublic.color) || null,
                                guid: data.guid
                            },
                            html: '<img src="' + data.url + '" style="max-width:200px;max-height:200px;display:block;margin-top:4px;" />',
                            timestamp: moment()
                        };
                        ...
                        if (!data || !data.src || !data.src.mp4) return;
                        var _bonzi = _.get(window.bonzis, data.guid) || null;
                        var pl = {
                            type: "media",
                            subtype: "video",
                            ...
                            html: '<video src="' + data.src.mp4 + '" autoplay loop style="max-width:250px;display:block;margin-top:4px;"></video>',
                            timestamp: moment()
                        };
                        $scope.addChatMessage(pl, {
                            html: !0
        ...
        function() {
            ...
            function EditRoomController($rootScope, $scope, $log, $http, $timeout, $interval, $window, socket, Room) {
                ...
                vm.isNewRoom = !1, vm.editRoom = null, vm.masterRoom = null, $scope.editId = null, vm.inProgress = !1, vm.joiningRoom = !1, $scope.initialized = !1, $scope.initialize = function(force) {
                    ...
                    if (vm.inProgress || vm.joiningRoom) return;
                    var roomName = (vm.editRoom && vm.editRoom.name && vm.editRoom.name.trim()) || '';
                    if (!roomName) {
                        window.showToast('error', 'Please enter a room name.');
                        return;
                    }
                    var rid = roomName.toLowerCase().replace(/[^a-z0-9_-]/g, '-').replace(/-+/g, '-');
                    var passcode = (vm.editRoom && vm.editRoom.preferences && vm.editRoom.preferences.passcode) || null;
                    $scope.doJoinRoom({
                        _id: rid,
                        code: rid
                    }, passcode);
