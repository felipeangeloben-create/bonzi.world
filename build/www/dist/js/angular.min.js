...
/*!
  ...
  * Module      : AngularJS
  * Built       : 2021-07-06T12:44:52-0400
...
* @license MIT
*/
function(window,angular,undefined){"use strict";angular.module("ngIdle",["ngIdle.keepalive","ngIdle.idle","ngIdle.countdown","ngIdle.title","ngIdle.localStorage"]),angular.module("ngIdle.keepalive",[]).provider("Keepalive",function(){var options={http:null,interval:600};this.http=function(value){if(!value)throw new Error("Argument must be a string containing a URL, or an object containing the HTTP request configuration.");angular.isString(value)&&(value={url:value,method:"GET"}),value.cache=!1,options.http=value};var setInterval=this.interval=function(seconds){if(seconds=parseInt(seconds),isNaN(seconds)||seconds<=0)throw new Error("Interval must be expressed in seconds and be greater than 0.");options.interval=seconds};this.$get=["$rootScope","$log","$interval","$http",function($rootScope,$log,$interval,$http){var state={ping:null};function handleResponse(response){$rootScope.$broadcast("KeepaliveResponse",response.data,response.status)}function ping(){$rootScope.$broadcast("Keepalive"),angular.isObject(options.http)&&$http(options.http).then(handleResponse).catch(handleResponse)}return{_options:function(){return options},setInterval:setInterval,start:function(){return $interval.cancel(state.ping),state.ping=$interval(ping,1e3*options.interval),state.ping},stop:function(){$interval.cancel(state.ping)},ping:function(){ping()}}}]}),angular.module("ngIdle.idle",["ngIdle.keepalive","ngIdle.localStorage"]).provider("Idle",function(){var options={idle:1200,timeout:30,autoResume:"idle",interrupt:"mousemove keydown DOMMouseScroll mousewheel mousedown touchstart touchmove scroll",windowInterrupt:null,keepalive:!0},setTimeout=this.timeout=function(seconds){if(!1===seconds)options.timeout=0;else{if(!(angular.isNumber(seconds)&&seconds>=0))throw new Error("Timeout must be zero or false to disable the feature, or a positive integer (in seconds) to enable it.");options.timeout=seconds}};this.interrupt=function(events){options.interrupt=events},this.windowInterrupt=function(events){options.windowInterrupt=events};var setIdle=this.idle=function(seconds){if(seconds<=0)throw new Error("Idle must be a value in seconds, greater than 0.");options.idle=seconds};this.autoResume=function(value){options.autoResume=!0===value?"idle":!1===value?"off":value},this.keepalive=function(enabled){options.keepalive=!0===enabled},this.$get=["$interval","$log","$rootScope","$document","Keepalive","IdleLocalStorage","$window",function($interval,$log,$rootScope,$document,Keepalive,LocalStorage,$window){var state={idle:null,timeout:null,idling:!1,running:!1,countdown:null},id=(new Date).getTime();function startKeepalive(){options.keepalive&&(state.running&&Keepalive.ping(),Keepalive.start())}function stopKeepalive(){options.keepalive&&Keepalive.stop()}function toggleState(){state.idling=!state.idling;var name=state.idling?"IdleStart":"IdleEnd";state.idling?($rootScope.$broadcast(name),stopKeepalive(),options.timeout&&(state.countdown=options.timeout,countdown(),state.timeout=$interval(countdown,1e3,options.timeout,!1))):(startKeepalive(),$rootScope.$broadcast(name)),$interval.cancel(state.idle)}function countdown(){state.idling&&(state.countdown<=0?timeout():($rootScope.$broadcast("IdleWarn",state.countdown),state.countdown--))}function timeout(){stopKeepalive(),$interval.cancel(state.idle),$interval.cancel(state.timeout),state.idling=!0,state.running=!1,state.countdown=0,$rootScope.$broadcast("IdleTimeout")}function changeOption(self,fn,value){var reset=self.running();self.unwatch(),fn(value),reset&&self.watch()}function setExpiry(date){date?LocalStorage.set("expiry",{id:id,time:date}):LocalStorage.remove("expiry")}var svc={_options:function(){return options},_getNow:function(){return new Date},getIdle:function(){return options.idle},getTimeout:function(){return options.timeout},setIdle:function(seconds){changeOption(this,setIdle,seconds)},setTimeout:function(seconds){changeOption(this,setTimeout,seconds)},isExpired:function(){var obj,expiry=(obj=LocalStorage.get("expiry"))&&obj.time?new Date(obj.time):null;return null!==expiry&&expiry<=this._getNow()},running:function(){return state.running},idling:function(){return state.idling},watch:function(noExpiryUpdate){$interval.cancel(state.idle),$interval.cancel(state.timeout);var timeout=options.timeout?options.timeout:0;noExpiryUpdate||setExpiry(new Date((new Date).getTime()+1e3*(options.idle+timeout))),state.idling?toggleState():state.running||startKeepalive(),state.running=!0,state.idle=$interval(toggleState,1e3*options.idle,0,!1)},unwatch:function(){$interval.cancel(state.idle),$interval.cancel(state.timeout),state.idling=!1,state.running=!1,setExpiry(null),stopKeepalive()},interrupt:function(anotherTab){state.running&&(options.timeout&&this.isExpired()?timeout():(!function(anotherTab){$rootScope.$broadcast("IdleInterrupt",anotherTab)}(anotherTab),(anotherTab||"idle"===options.autoResume||"notIdle"===options.autoResume&&!state.idling)&&this.watch(anotherTab)))}},lastMove={clientX:null,clientY:null,swap:function(event){var last={clientX:this.clientX,clientY:this.clientY};return this.clientX=event.clientX,this.clientY=event.clientY,last},hasMoved:function(event){var last=this.swap(event);return!(null!==this.clientX&&!event.movementX&&!event.movementY)||(last.clientX!=event.clientX||last.clientY!=event.clientY)}};if($document.find("html").on(options.interrupt,function(event){"mousemove"===event.type&&event.originalEvent&&0===event.originalEvent.movementX&&0===event.originalEvent.movementY||("mousemove"!==event.type||lastMove.hasMoved(event))&&svc.interrupt()}),options.windowInterrupt)for(var eventList=options.windowInterrupt.split(" "),fn=function(){svc.interrupt()},i=0;i<eventList.length;i++)$window.addEventListener?$window.addEventListener(eventList[i],fn,!1):$window.attachEvent(eventList[i],fn);var wrap=function(event){if("ngIdle.expiry"===event.key&&event.newValue&&event.newValue!==event.oldValue){if(angular.fromJson(event.newValue).id===id)return;svc.interrupt(!0)}};return $window.addEventListener?$window.addEventListener("storage",wrap,!1):$window.attachEvent&&$window.attachEvent("onstorage",wrap),svc}]}),angular.module("ngIdle.countdown",["ngIdle.idle"]).directive("idleCountdown",["Idle",function(Idle){return{restrict:"A",scope:{value:"=idleCountdown"},link:function($scope){$scope.value=Idle.getTimeout(),$scope.$on("IdleWarn",function(e,countdown){$scope.$evalAsync(function(){$scope.value=countdown})}),$scope.$on("IdleTimeout",function(){$scope.$evalAsync(function(){$scope.value=0})})}}}]),angular.module("ngIdle.title",[]).provider("Title",function(){var options={enabled:!1},setEnabled=this.enabled=function(enabled){options.enabled=!0===enabled};this.$get=["$document","$interpolate",function($document,$interpolate){var state={original:null,idle:"{{minutes}}:{{seconds}} until your session times out!",timedout:"Your session has expired."};return{setEnabled:setEnabled,isEnabled:function(){return options.enabled},original:function(val){if(angular.isUndefined(val))return state.original;state.original=val},store:function(overwrite){!overwrite&&state.original||(state.original=this.value())},value:function(val){if(angular.isUndefined(val))return $document[0].title;$document[0].title=val},idleMessage:function(val){if(angular.isUndefined(val))return state.idle;state.idle=val},timedOutMessage:function(val){if(angular.isUndefined(val))return state.timedout;state.timedout=val},setAsIdle:function(countdown){this.store();var nr,str,remaining={totalSeconds:countdown};remaining.minutes=Math.floor(countdown/60),remaining.seconds=(nr=countdown-60*remaining.minutes,new Array(2-String(nr).length+1).join(str||"0")+nr),this.value($interpolate(this.idleMessage())(remaining))},setAsTimedOut:function(){this.store(),this.value(this.timedOutMessage())},restore:function(){this.original()&&this.value(this.original())}}}]}).directive("title",["Title",function(Title){return{restrict:"E",link:function($scope,$element,$attr){Title.isEnabled()&&!$attr.idleDisabled&&(Title.store(!0),$scope.$on("IdleStart",function(){Title.original($element[0].innerText)}),$scope.$on("IdleWarn",function(e,countdown){Title.setAsIdle(countdown)}),$scope.$on("IdleEnd",function(){Title.restore()}),$scope.$on("IdleTimeout",function(){Title.setAsTimedOut()}))}}}]),angular.module("ngIdle.localStorage",[]).service("IdleStorageAccessor",["$window",function($window){return{get:function(){return $window.localStorage}}}]).service("IdleLocalStorage",["IdleStorageAccessor",function(IdleStorageAccessor){var storage=function(){try{var s=IdleStorageAccessor.get();return s.setItem("ngIdleStorage",""),s.removeItem("ngIdleStorage"),s}catch(err){return new function(){var storageMap={};this.setItem=function(key,value){storageMap[key]=value},this.getItem=function(key){return void 0!==storageMap[key]?storageMap[key]:null},this.removeItem=function(key){storageMap[key]=undefined}}}}();return{set:function(key,value){storage.setItem("ngIdle."+key,angular.toJson(value))},get:function(key){return angular.fromJson(storage.getItem("ngIdle."+key))},remove:function(key){storage.removeItem("ngIdle."+key)},_wrapped:function(){return storage}}}])}(window,window.angular);
//# sourceMappingURL=angular.min.js.map
/*!  https://bonzi.world  !*/
(function(){window._jsModules=window._jsModules||{};window._jsModules.angularjs={module:'AngularJS',version:'4.63.3896',release:'291bdf88738563ba142c0d34a57747ed932d6a6e',built:'2021-07-06T12:44:52-0400',environment:'development'};})();


}
/*
     FILE ARCHIVED ON 02:18:56 Dec 26, 2021 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 17:09:59 May 22, 2022.
...
/*
playback timings (ms):
  captures_list: 219.492
  exclusion.robots: 0.112
  exclusion.robots.policy: 0.104
  cdx.remote: 0.072
  esindex: 0.01
  LoadShardBlock: 182.779 (3)
  PetaboxLoader3.datanode: 94.43 (4)
  CDXLines.iter: 18.115 (3)
  PetaboxLoader3.resolve: 110.858 (2)
  load_resource: 128.612
*/
