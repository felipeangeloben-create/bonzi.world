...
socket.on("dvdbounce", function (a) {
    ...
    if (!b) return;
    b.cancel();
    b.rng = new Math.seedrandom(a.rng);
    var $el = b.$element || b.element;
    if (!$el) return;
    var speed = 4, dx = b.rng() > 0.5 ? speed : -speed, dy = b.rng() > 0.5 ? speed : -speed;
    if (b.bounceInterval) clearInterval(b.bounceInterval);
    b.bounceInterval = setInterval(function () {
        var x = parseFloat($el.css("left")) || 0, y = parseFloat($el.css("top")) || 0;
        var elW = $el.outerWidth() || 100, elH = $el.outerHeight() || 100;
        var winW = $(window).width(), winH = $(window).height();
        var nX = x + dx, nY = y + dy;
        if (nX <= 0 || nX + elW >= winW) dx = -dx;
        if (nY <= 0 || nY + elH >= winH) dy = -dy;
        $el.css({ left: (x + dx) + "px", top: (y + dy) + "px" });
    }, 16);
}),
