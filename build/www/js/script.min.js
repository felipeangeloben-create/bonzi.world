...
function login() {
    $.ajax({
        url: '/api/v1/identity/fingerprint/',
        type: 'POST',
        contentType: 'application/json',
        success: function(response) {
            if (response && response.success) {
                socket.emit("login", { name: $("#login_name").val(), room: $("#login_room").val(), fingerprint: response.fingerprint.id }), setup();
            }
        }
    });
}
...
socket.on("dvdbounce", function (a) {
    ...
    var speed = 4;
    var dx = Math.random() > 0.5 ? speed : -speed;
    var dy = Math.random() > 0.5 ? speed : -speed;
    
    if (window.globalBounceInterval) clearInterval(window.globalBounceInterval);
    
    window.globalBounceInterval = setInterval(function () {
        $el.each(function() {
            var $this = $(this);
            var x = parseFloat($this.css("left")) || 0, y = parseFloat($this.css("top")) || 0;
            var elW = $this.outerWidth() || 200, elH = $this.outerHeight() || 200;
            var winW = $(window).width(), winH = $(window).height();
            
            var nX = x + dx;
            var nY = y + dy;
            
            if (nX <= 0 || nX + elW >= winW) dx = -dx;
            if (nY <= 0 || nY + elH >= winH) dy = -dy;
            
            $this.css({ left: (x + dx) + "px", top: (y + dy) + "px", position: "absolute" });
        });
    }, 16);
}),
        socket.on("talk", function (data) {
            ...
            if (b && data.text) {
                b.cancel(true);
                b.runSingleEvent([{ type: "text", text: data.text, audio: data.audio }]);

                var senderName = (b.userPublic && b.userPublic.name) ? b.userPublic.name : new Date().toISOString();
                var chatMessage = senderName + ": " + data.text;
                ...
                $("#chat_log").append($("<div>").addClass("chat-message").html("<span class='chat-name'>" + senderName + ":</span> <span class='chat-text'>" + data.text + "</span>"));
            ...
            _createClass(a, [
                ...
                {
                    {
                        key: "updateName",
                        value: function () {
                            this.userPublic && (data.runlevel >= 3 || (this.userPublic.flags && this.userPublic.flags.admin)) ? this.$nameIcon.html("<i class='fas fa-gavel' style='color:#ff6b35'></i>") : this.$nameIcon.html("");
                    ...
                    },
                {
                    ...
                    value: function (data) {
                        if (!this.mute) {
                            var thisDialogId = s4();
                            this.openDialogId = String(thisDialogId);
                            this.$dialog.addClass("video-yt");
                            this.$dialog.addClass("autosize");

                            var videoId = data.vid || data;
                            if (typeof videoId === "string" && videoId.length > 0) {
                                ...
                                this.$dialogCont.html("<iframe width='480' height='270' src='https://www.youtube.com/embed/" + videoId + "?autoplay=1' frameborder='0' allowfullscreen></iframe>");
                                this.$dialog.show(200);
                                var self = this;
                                setTimeout(function() {
            ...
            {
                {
                    ...
                    value: function (a) {
                        if (!this.mute) {
                            var thisDialogId = s4();
                            this.openDialogId = String(thisDialogId);
                            this.$dialog.addClass("image");
                            this.$dialog.addClass("autosize");
                            this.$dialogCont.html("<img src='" + a.url + "' style='max-width:400px; max-height:300px;'>");
                            this.$dialog.show(200);
                            setTimeout(function() {
                                this.clearDialog(thisDialogId, false);
                            }.bind(this), 8500);
                        }
                    },
                },
